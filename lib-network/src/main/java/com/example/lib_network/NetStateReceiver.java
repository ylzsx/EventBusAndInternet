...
import com.example.lib_network.bean.MethodManager;
import com.example.lib_network.type.NetType;
import com.example.lib_network.utils.NetworkUtil;
...
//                if (listener != null) listener.onDisConnect();
    ...
    private List<MethodManager> findMethod(Object object) {
        ...
        for (Method declaredMethod : declaredMethods) {
            Network annotation = declaredMethod.getAnnotation(Network.class);
            if (annotation == null) {
                continue;
            }
            Class<?>[] parameterTypes = declaredMethod.getParameterTypes();
            if (parameterTypes.length != 1) {
                throw new RuntimeException(declaredMethod.getName() + "应该有一个参数");
            }
            if (!parameterTypes[0].isAssignableFrom(NetType.class)) {
                throw new RuntimeException(declaredMethod.getName() + "参数类型应该为 NetType");
            }
            MethodManager methodManager = new MethodManager(parameterTypes[0], annotation.netType(), declaredMethod);
